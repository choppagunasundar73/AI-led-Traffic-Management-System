...
# # Define the horizontal line for counting
# line_y = int(frame_height * 0.1)

# # NEW: Define a vertical boundary line (x-bound)
# # We will only count vehicles to the right of this line
# x_bound = int(frame_width * 0.4) # Positioned at the halfway mark

# # Store the track history and counted vehicle IDs
...
#         # --- NEW: Draw the boundary lines ---
#         # Draw the vertical boundary line
#         cv2.line(annotated_frame, (x_bound, 0), (x_bound, frame_height), (255, 255, 0), 2) # Cyan color
#         # Draw the horizontal counting line only in the active region
...
#             # Manage track history
#             track = track_history[track_id]
#             track.append(center_y)
#             if len(track) > 2:
#                 track.pop(0)

#             # --- UPDATED: The Counting Logic ---
#             # Check if the vehicle has crossed the line AND is in the correct region
#             if len(track) == 2 and track_id not in counted_ids:
#                 # Check for crossing downwards AND if its x-center is right of the boundary
#                 if track[0] < line_y and track[1] >= line_y and center_x > x_bound:
#                     vehicle_count += 1
#                     counted_ids.add(track_id)
#                     # Briefly change the line to red when a vehicle is counted
#                     cv2.line(annotated_frame, (x_bound, line_y), (frame_width, line_y), (0, 0, 255), 3)

#     else:
#         annotated_frame = frame
#         # Draw boundary lines even if no objects are detected
#         cv2.line(annotated_frame, (x_bound, 0), (x_bound, frame_height), (255, 255, 0), 2)
#         cv2.line(annotated_frame, (x_bound, line_y), (frame_width, line_y), (0, 255, 0), 3)

#     # Display the total vehicle count
#     cv2.putText(annotated_frame, f'Vehicle Count: {vehicle_count}', (50, 50), 
#                 cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 255, 255), 3, cv2.LINE_AA)

#     cv2.imshow("YOLOv8 Vehicle Counter", annotated_frame)
